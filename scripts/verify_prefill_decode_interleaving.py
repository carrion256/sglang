#!/usr/bin/env python3
"""Verify the standalone Prefill/decode interleaving patch profile."""
...
def base_inventory() -> dict[str, str]:
    manifest = json.loads((ROOT / "provenance/prefill-decode-interleaving.json").read_text())
...
def package_records() -> tuple[dict, dict[str, str]]:
    manifest_path = ROOT / "provenance/prefill-decode-interleaving.json"
    ...
    series = (ROOT / "patches/series.prefill-decode-interleaving").read_text().splitlines()
    if series != [row["file"] for row in manifest["patches"]]:
        raise ValueError("Prefill/decode interleaving patch order differs")
    ...
    for patch_record in manifest["patches"]:
        ...
        if digest(patch) != patch_record["sha256"]:
            raise ValueError("Prefill/decode interleaving patch hash mismatch: " + patch.name)
        for name, hashes in patch_record["files"].items():
            if inventory.get(name) != hashes["before"]:
                raise ValueError("Prefill/decode interleaving patch transition mismatch: " + name)
            inventory[name] = hashes["after"]
            changed_paths.add(name)

    if changed_paths != set(manifest["files"]):
        raise ValueError("Prefill/decode interleaving changed-path manifest differs")
    for name, hashes in manifest["files"].items():
        if initial.get(name) != hashes["before"] or inventory[name] != hashes["after"]:
            raise ValueError("Prefill/decode interleaving cumulative file transition mismatch: " + name)
...
if __name__ == "__main__":
    ...
    print(
        json.dumps(
            {
                "profile": "prefill-decode-interleaving",
