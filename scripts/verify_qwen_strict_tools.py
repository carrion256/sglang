#!/usr/bin/env python3
"""Verify the standalone Qwen strict tools patch profile."""
...
def base_inventory() -> dict[str, str]:
    manifest = json.loads((ROOT / "provenance/qwen-strict-tools.json").read_text())
...
def package_records() -> tuple[dict, dict[str, str]]:
    manifest_path = ROOT / "provenance/qwen-strict-tools.json"
    ...
    series = (ROOT / "patches/series.qwen-strict-tools").read_text().splitlines()
    if series != [row["file"] for row in manifest["patches"]]:
        raise ValueError("Qwen strict tools patch order differs")
    ...
    for patch_record in manifest["patches"]:
        ...
        if digest(patch) != patch_record["sha256"]:
            raise ValueError("Qwen strict tools patch hash mismatch: " + patch.name)
        for name, hashes in patch_record["files"].items():
            if inventory.get(name) != hashes["before"]:
                raise ValueError("Qwen strict tools patch transition mismatch: " + name)
            inventory[name] = hashes["after"]
            changed_paths.add(name)

    if changed_paths != set(manifest["files"]):
        raise ValueError("Qwen strict tools changed-path manifest differs")
    for name, hashes in manifest["files"].items():
        if initial.get(name) != hashes["before"] or inventory[name] != hashes["after"]:
            raise ValueError("Qwen strict tools cumulative file transition mismatch: " + name)
