#!/usr/bin/env python3
"""Verify the isolated, opt-in PR#19 hicache patch profile."""
...
def base_inventory() -> dict[str, str]:
    manifest = json.loads((ROOT / "provenance/hicache-pr19-embed.json").read_text())
...
def package_records() -> tuple[dict, dict[str, str]]:
    manifest_path = ROOT / "provenance/hicache-pr19-embed.json"
    ...
    series = (ROOT / "patches/series.hicache-pr19-embed").read_text().splitlines()
    if series != [row["file"] for row in manifest["patches"]]:
        raise ValueError("PR#19 hicache patch order differs")
    ...
    for patch_record in manifest["patches"]:
        ...
        if digest(patch) != patch_record["sha256"]:
            raise ValueError("PR#19 hicache patch hash mismatch: " + patch.name)
        for name, hashes in patch_record["files"].items():
            if inventory.get(name) != hashes["before"]:
                raise ValueError("PR#19 hicache patch transition mismatch: " + name)
