#!/usr/bin/env python3
"""Verify the isolated, opt-in rvn-w4a16 patch profile."""
...
def base_inventory() -> dict[str, str]:
    manifest = json.loads((ROOT / "provenance/rvn-w4a16.json").read_text())
...
def package_records() -> tuple[dict, dict[str, str]]:
    manifest_path = ROOT / "provenance/rvn-w4a16.json"
    ...
    series = (ROOT / "patches/series.rvn-w4a16").read_text().splitlines()
    if series != [row["file"] for row in manifest["patches"]]:
        raise ValueError("rvn-w4a16 patch order differs")
    ...
    for patch_record in manifest["patches"]:
        ...
        if digest(patch) != patch_record["sha256"]:
            raise ValueError("rvn-w4a16 patch hash mismatch: " + patch.name)
        for name, hashes in patch_record["files"].items():
            if inventory.get(name) != hashes["before"]:
                raise ValueError("rvn-w4a16 patch transition mismatch: " + name)
            inventory[name] = hashes["after"]
            changed_paths.add(name)

    if changed_paths != set(manifest["files"]):
        raise ValueError("rvn-w4a16 changed-path manifest differs")
    for name, hashes in manifest["files"].items():
        if initial.get(name) != hashes["before"] or inventory[name] != hashes["after"]:
            raise ValueError("rvn-w4a16 cumulative file transition mismatch: " + name)
