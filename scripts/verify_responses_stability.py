#!/usr/bin/env python3
"""Verify the standalone Responses stability patch profile."""
...
def base_inventory() -> dict[str, str]:
    manifest = json.loads((ROOT / "provenance/responses-stability.json").read_text())
...
def package_records() -> tuple[dict, dict[str, str]]:
    manifest_path = ROOT / "provenance/responses-stability.json"
    ...
    series = (ROOT / "patches/series.responses-stability").read_text().splitlines()
    if series != [row["file"] for row in manifest["patches"]]:
        raise ValueError("Responses stability patch order differs")
    ...
    for patch_record in manifest["patches"]:
        ...
        if digest(patch) != patch_record["sha256"]:
            raise ValueError("Responses stability patch hash mismatch: " + patch.name)
        for name, hashes in patch_record["files"].items():
            if inventory.get(name) != hashes["before"]:
                raise ValueError("Responses stability patch transition mismatch: " + name)
            inventory[name] = hashes["after"]
            changed_paths.add(name)

    if changed_paths != set(manifest["files"]):
        raise ValueError("Responses stability changed-path manifest differs")
    for name, hashes in manifest["files"].items():
        if initial.get(name) != hashes["before"] or inventory[name] != hashes["after"]:
            raise ValueError("Responses stability cumulative file transition mismatch: " + name)
