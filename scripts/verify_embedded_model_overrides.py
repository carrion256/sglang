#!/usr/bin/env python3
"""Verify the standalone Embedded model overrides patch profile."""
...
def base_inventory() -> dict[str, str]:
    manifest = json.loads((ROOT / "provenance/embedded-model-overrides.json").read_text())
...
def package_records() -> tuple[dict, dict[str, str]]:
    manifest_path = ROOT / "provenance/embedded-model-overrides.json"
    ...
    series = (ROOT / "patches/series.embedded-model-overrides").read_text().splitlines()
    if series != [row["file"] for row in manifest["patches"]]:
        raise ValueError("Embedded model overrides patch order differs")
    ...
    for patch_record in manifest["patches"]:
        ...
        if digest(patch) != patch_record["sha256"]:
            raise ValueError("Embedded model overrides patch hash mismatch: " + patch.name)
        for name, hashes in patch_record["files"].items():
            if inventory.get(name) != hashes["before"]:
                raise ValueError("Embedded model overrides patch transition mismatch: " + name)
            inventory[name] = hashes["after"]
            changed_paths.add(name)

    if changed_paths != set(manifest["files"]):
        raise ValueError("Embedded model overrides changed-path manifest differs")
    for name, hashes in manifest["files"].items():
        if initial.get(name) != hashes["before"] or inventory[name] != hashes["after"]:
            raise ValueError("Embedded model overrides cumulative file transition mismatch: " + name)
...
if __name__ == "__main__":
    ...
    print(
        json.dumps(
            {
                "profile": "embedded-model-overrides",
