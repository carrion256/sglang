...
| 15 | `0061-rvn-shared-ple-identity-cleanup.patch` | Ties each shared-table filename to the checkpoint identity (a hash of the grafted `config.json`) so a different same-shape checkpoint gets its own file or the private path rather than silently rewriting a live replica's table; every post-register failure path now `cudaHostUnregister`s before unmapping, the runtime is resolved from the process's own loaded `libcudart` (never a foreign `CDLL("libcudart.so")`), and the device-pointer alias query uses that same handle (env unset ⇒ byte-identical to before) |
